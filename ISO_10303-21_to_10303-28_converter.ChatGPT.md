...
Q: Does an EXPRESS (.exp) to XML Schema (.xsd) converter exist?

Yes — but just like with Part 21 → Part 28, the situation depends on whether you’re looking for **official ISO Part 28 tooling** or a more practical implementation.

---

### Standards basis

* **ISO 10303-28 (STEP-XML)** explicitly defines how an EXPRESS schema (`.exp`) is mapped to an XML Schema (`.xsd`).
* In fact, one of the normative outputs of Part 28 is the XML Schema derived from an EXPRESS schema. So formally, *every* EXPRESS schema can be transformed into an XSD, following the mapping rules given in the standard.

---

### Existing converters / implementations

1. **StepTools SDK (Rose/STEP Tools)**

   * Provides functionality to generate XML Schema definitions from EXPRESS schemas, alongside STEP-XML serialization.
   * You load the EXPRESS (`.exp`) into their API and can export the corresponding `.xsd`. This is used in AP214, AP242, IFC, etc.

2. **JSDAI (Java Standard Data Access Interface)**

   * An implementation of EXPRESS/STEP in Java.
   * Includes facilities to map EXPRESS to XML Schema and work with STEP-XML.
   * Was developed under ISO TC184/SC4 support projects, still available in academic/industrial contexts.

3. **IFC toolchains (buildingSMART / xbim / IfcOpenShell)**

   * For the IFC schemas (which are EXPRESS-based), buildingSMART publishes both the `.exp` and the corresponding `.xsd` generated using the Part 28 mapping rules.
   * That implies that converters are in active use, though mostly schema-specific.

4. **Commercial toolkits (Datakit, Elysium, HOOPS Exchange, TransMagic)**

   * These toolkits offer XML export for AP schemas, which internally requires EXPRESS → XSD generation.
   * Direct user-facing “.exp → .xsd” may not be exposed, but the function exists inside their translators.

---

### Open-source status

* There isn’t a **single widely maintained open-source converter** that takes an arbitrary `.exp` file and emits `.xsd`.
* Projects like **STEPcode** or **Open CASCADE** have EXPRESS parsers, but their Part 28 modules are incomplete or not maintained — so you wouldn’t get a ready-to-use `.xsd` exporter out of the box.
* You *could* extend them: parse `.exp` with STEPcode’s EXPRESS parser, then implement the Part 28 XSD mapping rules yourself.

---

### Summary

✅ **Yes, converters exist.**

* Commercial / standards-oriented SDKs (StepTools, JSDAI) can generate `.xsd` from `.exp`.
* IFC workflows already use this (buildingSMART publishes `.xsd` generated from EXPRESS).
  ❌ **No universal open-source tool** is widely adopted — you’d need either a commercial toolkit or to build on EXPRESS parsers like STEPcode.

---

Would you like me to dig up **concrete download links / usage examples** (e.g. where to get JSDAI, or sample StepTools code showing `.exp → .xsd` export)?
